...
class Watchdog implements Runnable {
    ...
    static class CSVDatabase {
        private Map<String, User> users = new HashMap<>();

        public boolean addUser(String username, byte[] salt, String hashedPassword) {
...
class ChatClient {
    ...
    static class SocketReaderTask implements Runnable {
        ...
        private String hashPassword(String password, byte[] salt) {
            return Watchdog.CSVDatabase.hashPassword(password, salt);
        }
                ...
                while ((message = reader.readLine()) != null) {
                    switch (message.split(" ", 2)[0]) {
                        ...
                        case "#login":
                            ...
                            if (colonIndex2 != -1) {
                                String login = message.substring("#login ".length(), colonIndex2).trim();
                                String heslo = message.substring(colonIndex2 + 1).trim();

                                // Use the CSVDatabase instance to check login credentials
                                if (watchdog.csvDatabase.checkLogin(login, heslo)) {
                                    self.outputTask.sendMessage("Login successful.\r\n");
                                    self.nickname = login;
                                    // Add logic to handle successful login
                                } else {
                                    self.outputTask.sendMessage("Invalid login credentials.\r\n");
                                    // Add logic to handle failed login
                                }
                            }
                            break;

                        case "#createnewuser":
                            int colonIndex3 = message.indexOf(":");
                            if (colonIndex3 != -1) {
                                String newLogin = message.substring("#createnewuser ".length(), colonIndex3).trim();
                                String newPassword = message.substring(colonIndex3 + 1).trim();

                                // Generate a salt for the new user
                                String newSalt = generateSalt();

                                // Hash the password using SHA-512 and the generated salt
                                String hashedPassword = Watchdog.CSVDatabase.hashPassword(newPassword, newSalt.getBytes());

                                // Add the new user to the CSVDatabase
                                if(watchdog.csvDatabase.addUser(newLogin, newSalt.getBytes(), hashedPassword)) {
                                    self.outputTask.sendMessage("New user created successfully.\r\n");
                                    // Add logic to handle successful user creation
                                } else {
                                    self.outputTask.sendMessage("Failed to create a new user. The login may already exist.\r\n");
                                    // Add logic to handle failed user creation
                                }
                            }
                            break;
