...
class ChatClient {
    ...
    public class PostRequestDemo {
        ...
        public static void main(String[] args) {
            String payload = "{\"typStandardizaceAdresy\": \"VYHOVUJICI_ADRESY\", "
                    + "\"textovaAdresa\": \"zlin 4511\"}";
            HttpClient client = HttpClient.newHttpClient();

            HttpRequest request = HttpRequest.newBuilder(URI.create("https://ares.gov.cz/ekonomicke-subjekty-v-be/rest/standardizovane-adresy/vyhledat"))
            ...
            try {
                ...
                if (status==200) {
                    ...
                    String stavStandardizace = json.getString("stavStandardizace");

                    if ("NEUSPESNA".equals(stavStandardizace)) {
                        System.out.println("Adresa nebyla nalezena");
                    } else {
                        String textovaAdresa = json.getString("textovaAdresa");
                        System.out.println(textovaAdresa);
                    }
                    System.out.println(response.body());
                }
                else{
                    System.out.println("Chyba: ares.gov.cz vrátil HTTP response status " + status);
                } // TODO: pošlete uživateli zprávu ("Chyba: ares.gov.cz vrátil HTTP response status "+status)
            } catch (IOException | InterruptedException e) {
                System.out.println("Chyba: " + e.getMessage());
                // TODO pošlete uživateli zprávu o chybě, klidně e.getMessage()
            }
        }
                    ...
                    switch (message.split(" ", 2)[0]) {
                        ...
                        case "#pocasi":
                            try {
                                ...
                                HttpRequest weatherRequest = HttpRequest.newBuilder(URI.create("https://ares.gov.cz/ekonomicke-subjekty-v-be/rest/standardizovane-adresy/vyhledat"))
                                        .header("content-type", "application/json")
                                        .POST(HttpRequest.BodyPublishers.ofString(payload))
                                        .build();

                                HttpResponse<String> weatherResponse = client.send(weatherRequest, BodyHandlers.ofString());
                                int weatherStatus = weatherResponse.statusCode();
                                String weatherJsonResponse = weatherResponse.body();

                                System.out.println("Weather API Response: " + weatherJsonResponse);
                                JSONObject weatherJson = new JSONObject(weatherJsonResponse);
                                System.out.println(weatherJson);// Add this line
                                weatherJson.

                                if (weatherStatus == 200) {


                                    if (weatherJson.has("textovaAdresa")) {
                                        String weatherTextovaAdresa = weatherJson.getString("textovaAdresa");
                                        self.outputTask.sendMessage("Pocasi pro adresu: " + weatherTextovaAdresa + "\r\n");
                                        // Add additional processing for weather data if needed
                                    } else {
                                        self.outputTask.sendMessage("Chyba: Klíč 'textovaAdresa' nenalezen v odpovědi API\r\n");
                                    }
                                } else {
                                    self.outputTask.sendMessage("Chyba: ares.gov.cz vrátil HTTP response status " + weatherStatus + "\r\n");
                                }
                            } catch (IOException | InterruptedException e) {
                                self.outputTask.sendMessage("Chyba při získávání počasí: " + e.getMessage() + "\r\n");
                            }
                            break;
